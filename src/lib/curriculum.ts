...
export interface Lesson {
  ...
  // ── Tier 1 interactive features (v2.5 — RF/AC/power/mixed-signal sims) ─
  has_smith?: boolean;             // Show the interactive Smith chart (RF matching)
  has_phasor?: boolean;            // Show the interactive phasor diagram (AC analysis)
  has_pwm?: boolean;               // Show the PWM visualizer (power electronics)
  has_adc_dac?: boolean;           // Show the ADC/DAC sampling visualizer (mixed-signal)
  // ── Tier 1 interactive features (v2.6 — control-systems + digital-logic) ─
  has_root_locus?: boolean;        // Show the interactive root locus plot (Phase 6)
  has_nyquist?: boolean;           // Show the interactive Nyquist plot (Phase 6)
  has_kmap?: boolean;              // Show the Karnaugh-map solver (Phase 4)
  has_logic_analyzer?: boolean;    // Show the 8-channel logic analyzer (Phase 4)
  // ── Tier 1 interactive features (v2.7 — RF/EM + FSM + power-systems) ─────
  has_transline?: boolean;         // Show the transmission-line wave + SWR sim (Phase 8)
  has_antenna?: boolean;           // Show the antenna radiation-pattern viewer (Phase 8)
  has_fsm?: boolean;               // Show the FSM editor + simulator (Phase 4/6)
  has_powerflow?: boolean;         // Show the 5-bus power-flow simulator (Phase 10)
}
...
` },
      ...
      { id: 'p4m6', title: 'Microcontrollers: Arduino to STM32', description: 'MCU = CPU + RAM + flash + peripherals. Arduino abstracts; STM32 makes you deal with registers via HAL or LL. Clock trees, low-power modes.',
        ...
        lessons: [
          { id: 'p4m6l1', title: 'Arduino abstraction and its limits', type: 'reading', duration_min: 60, summary: 'pinMode, digitalWrite, analogRead, etc. wrap AVR register writes. digitalWrite takes ~5 μs because of pin-number lookup. For sub-μs timing, bypass the abstraction.', key_takeaways: ['Arduino is great for prototyping', 'Abstraction hides timing — digitalWrite is slow (~5 μs)', 'For tight timing, write registers directly'] , wokwi_url: '', has_wokwi_elements: true},
      ...
      { id: 'p4m8', title: 'Communication: UART, SPI, I2C, CAN', description: 'UART (point-to-point, 9600-3Mbaud). SPI (bus with CS, 1-50 MHz). I2C (addressed, 100kHz-3.4MHz). CAN (automotive, differential, 1 Mbit/s).',
        ...
        lessons: [
          { id: 'p4m8l1', title: 'Four protocols compared', type: 'reading', duration_min: 75, summary: 'UART: 2 wires (TX/RX), point-to-point, 9600-3Mbaud, debug serial. SPI: 4 wires (MOSI/MISO/SCK/CS), bus with CS per slave, 1-50 MHz, fast ADCs. I2C: 2 wires (SDA/SCL), multi-drop addressed, 100kHz-3.4MHz, sensors. CAN: 2 wires (H/L), differential, 1 Mbit/s, automotive.', key_takeaways: ['UART = point-to-point, simplest', 'SPI = fast, but CS per slave', 'I2C = addressed, only 2 wires', 'CAN = robust, differential, multi-master'], has_logic_analyzer: true, wavedrom: `{ signal: [\n  { name: 'SCK',  wave: '0..n......0.' },\n  { name: 'CS',   wave: '1.0.........1' },\n  { name: 'MOSI', wave: 'x.2345678x.', data: ['8','7','6','5','4','3','2','1'] },\n  { name: 'MISO', wave: 'x.abcdefghx.', data: ['a','b','c','d','e','f','g','h'] }\n],\n  head: { text: 'SPI: 8-bit full-duplex transaction (CS low enables slave)' },\n  foot: { text: 'Master clocks SCK; MOSI carries master→slave, MISO slave→master.' } }` },
